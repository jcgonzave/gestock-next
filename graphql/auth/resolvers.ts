...
import { getToken, getDataFromToken } from '../utils/auth';
...
const validateAndGetDataToken = (token) => {
  if (!token) {
...
const resolvers = {
  Query: {
    validatePasswordResetToken: (root, { token }) => {
      const data = validateAndGetDataToken(token);
      if (data === null) {
        return false;
      }
      return true;
  ...
  Mutation: {
    login: async (root, { email, password }, { prisma }) => {
      ...
      const token = getToken(currentUser, '30d');
    ...
    passwordRecovery: async (root, { email }, { prisma }) => {
      ...
      if (currentUser) {
        // await sendResetMail(currentUser, getToken);
      }
